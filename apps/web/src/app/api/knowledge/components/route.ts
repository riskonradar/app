...
export async function GET(request: Request) {
  ...
  const { data, error } = await (supabase.rpc as any)("get_component_taxonomy", {
    p_parent_slug: parentSlug,
  });

  if (error) {
    console.error("get_component_taxonomy error:", error);
    return Response.json({ error: error.message }, { status: 500 });
  }

  return Response.json({ components: (data ?? []).slice(0, limit) });
}
